...
function rowAction(navigate, buttonName, buttonUrl, state, user) {
    return {
        field: "action",
        ...
        renderCell: (params) => {
            const onClick = (e) => {
                ...
                try{
                    axiosApi
                    .get(`account/users/center/${thisRow.id}`)
                    .then((response) => {
                        staffs = response.data[0].id;
                        console.log(staffs);
                        console.log("Set staffs", staffs);
                    });
                }catch(err){
                    console.log(err)
                }
...
function DataGridSearchComponent(props) {
    ...
    let getData = async () => {
        ...
        console.log(sortby === "")
        console.log(gradeGte === "")
        console.log(gradeLte === "")
        if (searchTerm === "" && sortby === 0 && gradeGte === "" && gradeLte === "") {
            
            axiosApi.get(`/center/centers-appointment/${state}`).then((response) => {
                console.log("voidnvodoivnndf", response.data)
                setCenters(response.data);
            });
        } else{
            axiosApi
                .get(`/center/centers-appointment/${state}/?${searchTerm !=="" ? `search=${searchTerm}&` : ""}${sortby !=="" ? `ordering=${direction}${sortby}&` : ""}${gradeGte !="" ? `average_grade__gte=${gradeGte}&` : ""}${gradeLte !="" ? `average_grade__lte=${gradeLte}&` : ""}`) //${grade !="" ? `grade=${grade}&` : ""}
                .then((response) => {
                    setCenters(response.data);
                });}
    };
            ...
            <Paper>
                <Box sx={{ height: 700, width: "100%" }}>
                    <DataGrid
                        rows={centers}
                        disableColumnFilter
                        columns={[...columns, rowAction(navigate, props.buttonName, props.buttonUrl, state, user)]}
                        autoHeight
                        density="comfortable"
